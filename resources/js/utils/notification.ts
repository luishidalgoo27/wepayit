...
let fraseElegida = frasesCuñao[Math.floor(Math.random() * frasesCuñao.length)];

window.addEventListener("DOMContentLoaded", () => {
  const fraseElem = document.getElementById("frase-cuñao");

  if (fraseElem) {
    fraseElem.innerText = `"${fraseElegida}"`;
  }
});

export const getMessageCuñao = (): string => {
  return fraseElegida;
};